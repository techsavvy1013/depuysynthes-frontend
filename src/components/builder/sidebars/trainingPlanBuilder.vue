<template>
  <div class="px-2 pt-3 text-left">
    ...
    <div class="w-full absolute left-0 mt-4">
      <div
      ...
      >
        ...
        >
          <div class="flex items-center">
            ...
            />
            <p class="text-red-650 font-bold text-base tracking-wide">Timeline {{ i + 1 }}</p>
          </div>
          <SvgIcon
          ...
          <div class="p-5">
            ...
            <div class="flex justify-between items-center mt-3">
              ...
              <div class="w-6/12">
                ...
                <div class="flex items-center border relative rounded">
                  <input
                    v-model="timelines[i].endDate"
                    :name="`endDate-${i}`"
                    class="rounded font-bold focus:outline-none mb-0 py-2 px-2 text-sm w-full text-grey-darker"
                    type="date"
                    pattern="\d{4}-\d{2}-\d{2}"
                    :min="timelines[i].startDate"
                  >
                  <label
                    :for="`endDate-${i}`"
                    class="absolute right-0 top-2 arrow-icon w-5 h-5 mr-2 pointer-events-none"
            ...
            <div class="mt-3">
              ...
              <input
                v-model="timelines[i].label"
                class="border rounded font-bold py-2 px-2 text-sm w-full text-grey-darker"
                placeholder="Enter amount"
              >
            </div>
            <div
              v-for="(milestone, j) in timelines[i].milestones"
              :key="j"
            ...
            >
              <div class="w-6/12 h-full">
                ...
                <div class="flex items-center border relative">
                  <input
                    v-model="timelines[i].milestones[j].title"
                    class="rounded font-bold focus:outline-none mb-0 py-2 px-2 text-sm w-full text-grey-darker"
                  >
                </div>
              </div>
              <div class="w-6/12">
                ...
                >
                  <input
                    v-model="timelines[i].milestones[j].date"
...
export default {
  data() {
    return {
      header: trainingPlanDefault.header,
      timelines: trainingPlanDefault.timelines
    };
  },
  computed: {
    ...mapGetters({
      storeTimelines: "trainingPlan/timelines",
    }),
  },
  ...
  methods: {
    ...mapMutations({
      setTimelines: "trainingPlan/setTimelines",
      setHeader:"trainingPlan/setHeader",
    }),
    addMilestone(index) {
      this.timelines[index].milestones.push({ title: "", date: "" });
    },
    toggleModal() {
